			...
			case WM_COMMAND:
				switch(LOWORD(wParam))
				{
					case ID_BUTTON_PRESS:
						MessageBox(hwnd, TEXT("Hi!"), TEXT("This is a message"), MB_OK | MB_ICONEXCLAMATION);
					break;
					case ID_BUTTON_OTHER:
						MessageBox(hwnd, TEXT("Bye!"), TEXT("This is also a message"), MB_OK | MB_ICONEXCLAMATION);
					break;
				}
				break;
				default:
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
		case WM_CREATE:
		{
			...
			hSubMenu = CreatePopupMenu();
			AppendMenu(hSubMenu, MF_STRING, ID_HELP_ABOUT, TEXT("&About"));
			AppendMenu(hMenu, MF_STRING | MF_POPUP, (UINT)hSubMenu, TEXT("&Help"));
			//menu DIALOG (SHOW and HIDE)
			hSubMenu = CreatePopupMenu();
			AppendMenu(hSubMenu, MF_STRING, ID_DIALOG_SHOW, TEXT("&SHOW"));
			AppendMenu(hSubMenu, MF_STRING, ID_DIALOG_HIDE, TEXT("&HIDE"));
			AppendMenu(hMenu, MF_STRING | MF_POPUP, (UINT)hSubMenu, TEXT("&Dialog"));
			SetMenu(hwnd, hMenu);
			hIcon = LoadImage(NULL, TEXT("icon.ico"), IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
			if(hIcon){
				SendMessage(hwnd, WM_SETICON, ICON_BIG, (LPARAM)hIcon);
			}else{
				MessageBox(hwnd, TEXT("could not load icon!"), TEXT("Error"), MB_OK | MB_ICONERROR);
			}
			hIconSm = LoadImage(NULL, TEXT("icon.ico"), IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
			if(hIconSm){
				SendMessage(hwnd, WM_SETICON, ICON_SMALL, (LPARAM)hIconSm);
			}else{
				MessageBox(hwnd, TEXT("could not load the icon!"), TEXT("Error"), MB_OK | MB_ICONERROR);
			}			
			//Toolbar
			...
			if(g_hToolbar!=NULL){
				...
				MessageBox(hwnd, TEXT("g_hToolbar: CreateDialog return NULL"), TEXT("Warning!"), MB_OK | MB_ICONINFORMATION );
			}
        ...
        if(hEdit == NULL)
			...
			hButtonSomething = CreateWindowEx(0, TEXT("BUTTON"), TEXT("Click me now"),
			...
			switch(LOWORD(wParam))
			{
				case ID_HELP_ABOUT:
				{
					...
					if(ret == IDOK){
						...
						MessageBox(hwnd, TEXT("Dialog feiled!"), TEXT("Error"), MB_OK | MB_ICONINFORMATION );
					}
				}break;				
				case ID_FILE_EXIT:
